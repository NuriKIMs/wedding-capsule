...
import { getFirestore, collection, addDoc, deleteDoc, doc, setDoc, updateDoc, onSnapshot, query, serverTimestamp } from 'firebase/firestore';
...
const DEFAULT_PHASE_DATA = {
  ...
  '10s': {
    ...
    description: '눈물 없이는 볼 수 없는 그녀의 중2병 시절. 버디버디 아이디 기억나니?',
  },
  '20s': {
    title: '20대: 광란의 대학생활',
  ...
  '30s': {
    ...
    description: '이제 좀 어른 같나 싶었는데 결혼이라니! 그래도 제일 예쁜 우리 미소.',
  },
  'video': {
...
const DEFAULT_BGM_SETTINGS = {
    title: '프리스타일 - Y',
    url: 'https://t1.daumcdn.net/cfile/tistory/994C3F3359CF517D16?original'
};
...
const RetroCard = ({ children, className = "" }) => (
  <div className={`bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 ${className}`}>
    {children}
  </div>
);

const RetroButton = ({ children, onClick, disabled, variant = 'primary', className = "" }) => {
  const baseStyle = "w-full py-3 font-bold border-2 border-black active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-2";
  const variants = {
    primary: "bg-[#FF00FF] text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-[#D900D9]",
    secondary: "bg-[#00FFFF] text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-[#00E6E6]",
    action: "bg-[#FFFF00] text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-[#E6E600]",
    danger: "bg-red-500 text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-red-600",
    disabled: "bg-gray-300 text-gray-500 cursor-not-allowed shadow-none border-gray-400"
  };
  return (
...
const DetailModal = ({ item, items, onClose, onChange }) => {
  ...
  const commentText = item.comment.includes(':') ? item.comment.split(':').slice(1).join(':').trim() : item.comment;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-80 backdrop-blur-sm animate-fade-in">
      ...
      <div className="bg-white w-full max-w-sm border-2 border-black shadow-[8px_8px_0px_0px_#FF00FF] relative flex flex-col max-h-[90vh]">
        <div className="bg-[#000080] text-white p-2 flex justify-between items-center">
          ...
          <button onClick={onClose} className="hover:bg-red-500 p-1 transition-colors"><X size={16} /></button>
        </div>
          ...
          <div className="mt-4 bg-[#F5F5F5] border border-gray-300 p-3 relative">
              <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                       <p className="text-sm font-medium leading-snug">{commentText}</p>
        ...
        <div className="p-3 border-t-2 border-gray-100 bg-gray-50 text-center flex justify-between items-center text-xs text-gray-400">
          ...
          <button onClick={onClose} className="w-1/2 py-2 bg-black text-white font-bold text-sm hover:bg-gray-800 ml-auto">닫기</button>
        </div>
      </div>
    </div>
  );
...
const BGMEditModal = ({ isOpen, onClose, initialData, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black bg-opacity-80 backdrop-blur-sm animate-fade-in">
            <div className="bg-[#FFF0F5] w-full max-w-sm border-2 border-black shadow-[8px_8px_0px_0px_#FFFF00] p-4 space-y-4">
                <div className="flex justify-between items-center border-b-2 border-gray-300 pb-2">
                    <h3 className="font-black text-lg flex items-center gap-2"><Music size={20} /> BGM 설정</h3>
                    <button onClick={onClose}><X/></button>
                </div>
                <div className="space-y-3">
                    <div>
                        <label className="text-xs font-bold block mb-1">노래 제목</label>
                        <input type="text" className="w-full border-2 border-black p-2 bg-white" value={data.title} onChange={(e) => setData({...data, title: e.target.value})} placeholder="예: 프리스타일 - Y" />
                    </div>
                    <div>
                        <label className="text-xs font-bold block mb-1">MP3 주소</label>
                        <input type="text" className="w-full border-2 border-black p-2 bg-white" value={data.url} onChange={(e) => setData({...data, url: e.target.value})} placeholder="https://..." />
                        <p className="text-[10px] text-gray-500 mt-1">* 저작권 문제가 없는 MP3 파일의 URL을 입력하세요.</p>
                    </div>
                </div>
                <RetroButton onClick={() => onSave(data)} variant="primary"><Save size={18} /> 설정 저장하기</RetroButton>
            </div>
        </div>
    );
};
...
const PhaseEditModal = ({ isOpen, onClose, phaseKey, initialData, onSave }) => {
  ...
  useEffect(() => {
    if (initialData) setData({ ...initialData, videoSrc: initialData.videoSrc || '' });
  }, [initialData, isOpen]);
...
const UploadModal = ({ isOpen, onClose, onUpload, isUploading, initialData = null }) => {
  const [post, setPost] = useState({ title: '', author: '김누리', comment: '', phase: '10s', image: null });
  ...
  useEffect(() => {
    if (isOpen) {
        if (initialData) {
            const commentBody = initialData.comment.includes(':') 
                ? initialData.comment.split(':').slice(1).join(':').trim() 
                : initialData.comment;
            setPost({
            ...
            setPost({ title: '', author: '김누리', comment: '', phase: '10s', image: null });
        }
    }
  }, [isOpen, initialData]);
        ...
        <div className="bg-[#FFF0F5] w-full max-w-sm border-2 border-black shadow-[8px_8px_0px_0px_#00FFFF] p-4 space-y-4">
            <div className="flex justify-between items-center border-b-2 border-gray-300 pb-2">
                <h3 className="font-black text-lg flex items-center gap-2">
                    <Camera size={20} /> {isEditMode ? '추억 수정 (Admin)' : '추억 업로드 (Admin)'}
                </h3>
                <button onClick={onClose}><X/></button>
            </div>
            {isUploading ? (
                <div className="h-64 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p className="font-bold">{isEditMode ? '수정 내용 저장 중...' : '추억 저장 중...'}<br/>(사진 압축하는 중!)</p>
                </div>
            ) : (
            <div className="space-y-3">
                <div>
                ...
                <div>
                    ...
                    <div onClick={() => fileInputRef.current?.click()} className="w-full h-24 border-2 border-dashed border-black bg-white flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 relative overflow-hidden">
                        {post.image ? (
                            <img src={post.image} alt="Preview" className="h-full w-full object-contain" />
                        ) : (
                            <><Upload size={20} className="mb-1 text-gray-400" /><span className="text-xs text-gray-500 font-bold">사진 선택</span></>
                        )}
                         {isEditMode && post.image && (
                             <div className="absolute bottom-0 right-0 bg-black text-white text-[10px] px-1 opacity-70">클릭하여 변경</div>
                        )}
                    </div>
                </div>
                <div>
            ...
            {!isUploading && (
                <RetroButton onClick={handleSubmit} variant="action">
                    <Save size={18} /> {isEditMode ? '수정 완료' : '저장하기'}
                </RetroButton>
            )}
        </div>
    </div>
...
export default function TimeCapsuleApp() {
  ...
  const [editingItem, setEditingItem] = useState(null); // 수정할 아이템 데이터
  const [editingPhase, setEditingPhase] = useState(null); 
  
  // BGM Settings
  const [showBgmModal, setShowBgmModal] = useState(false);
  const [bgmSettings, setBgmSettings] = useState(DEFAULT_BGM_SETTINGS);
    ...
    const unsubscribe = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'phase_settings'), (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            if (doc.id === 'global_settings') {
                 ...
                 const data = doc.data();
                 if (data.bgm) {
                     setBgmSettings(data.bgm);
                 }
            } else if (doc.id !== 'phases') { 
  ...
  useEffect(() => {
      if (!audioRef.current) {
          audioRef.current = new Audio(bgmSettings.url);
          audioRef.current.loop = true;
          audioRef.current.volume = 0.5;
      } else if (audioRef.current.src !== bgmSettings.url) {
          // URL Changed
          const wasPlaying = !audioRef.current.paused;
          audioRef.current.src = bgmSettings.url;
          if (wasPlaying) {
  ...
  const handleBgmUpdate = async (newSettings) => {
      if (!db) return;
      try {
          await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'phase_settings', 'global_settings'), {
              bgm: newSettings
          }, { merge: true });
          setShowBgmModal(false);
      } catch (error) {
          console.error("Failed to update BGM", error);
          window.alert("BGM 설정 저장 실패");
      }
  };
  ...
  const handleNextStage = () => {
    if (progress <= currentTab) {
      ...
      if (currentTab < phaseData.phases.length - 1) setCurrentTab(currentTab + 1);
    }
  };
  ...
  const handleUploadOrUpdate = async (postData) => {
    if (!user || !db) return;
    setIsUploading(true);
    try {
        let finalImageSrc = postData.image;
        
        if (postData.image && postData.image.startsWith('data:')) {
             if (postData.image.length > 100000) {
                 finalImageSrc = await resizeImage(postData.image);
             }
        }
        
        if (!finalImageSrc) {
            finalImageSrc = `https://placehold.co/400x300/${Math.floor(Math.random()*16777215).toString(16)}/ffffff?text=${encodeURIComponent(postData.title)}`;
        }

        const commonData = {
            title: postData.title,
            src: finalImageSrc,
            comment: `${postData.author}: ${postData.comment}`,
            author: postData.author,
            avatarColor: postData.author === '김누리' ? '#FF69B4' : postData.author === '이주희' ? '#00BFFF' : '#FFA500',
            phase: postData.phase,
        };

        if (editingItem) {
            await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'memories', editingItem.id), {
                ...commonData
            });
            setEditingItem(null);
        } else {
        ...
        const phaseIndex = phaseData.phases.indexOf(postData.phase);
  ...
  const renderLogin = () => (
    <div className="min-h-screen bg-[#FFF0F5] flex flex-col items-center justify-center p-6 font-mono relative overflow-hidden">
      <div className="absolute top-10 left-10 opacity-20 transform -rotate-12"><PixelAvatar size={80} color="#FF00FF" /></div>
      ...
      <div className="w-full max-w-md space-y-8 z-10">
        ...
        <RetroCard className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500">유미소 본인만 접근 가능합니다.</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" maxLength="4" placeholder="CODE" className="w-full text-center text-3xl font-black py-4 border-b-4 border-black bg-transparent focus:outline-none focus:border-[#00FFFF] tracking-[1rem]" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} />
            {errorMsg && <p className="text-red-500 text-sm font-bold text-center animate-pulse">{errorMsg}</p>}
            <RetroButton onClick={handleLogin}>잠금 해제</RetroButton>
            <p className="text-xs text-center text-gray-400 mt-2">Hint: 1004(User) / 7979(Admin)</p>
          </form>
        </RetroCard>
      </div>
    </div>
    ...
    <div className="sticky top-0 z-40 bg-[#FFF0F5] border-b-2 border-black p-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
      <div className="flex space-x-2 max-w-md mx-auto">
        {phaseData.phases.map((phase, idx) => {
          ...
          return (
            <button key={phase} disabled={!isUnlocked} onClick={() => setCurrentTab(idx)} className={`px-4 py-2 font-bold text-sm border-2 border-black transition-all flex items-center space-x-1 rounded-none ${isActive ? 'bg-[#FF00FF] text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] -translate-y-1' : ''} ${!isActive && isUnlocked ? 'bg-white text-black hover:bg-gray-100' : ''} ${!isUnlocked ? 'bg-gray-300 text-gray-500 cursor-not-allowed border-gray-400' : ''}`}>
              {!isUnlocked && <Lock size={12} className="mr-1" />}
              {isUnlocked && phase === 'video' && <Play size={12} className="mr-1" />}
              <span>{label}</span>
            </button>
          );
        })}
    ...
    return (
      <div className="space-y-6 pb-24 px-4">
        ...
        <div className="grid grid-cols-2 gap-4">
                ...
                {isAdmin && (
                    <div className="absolute top-1 right-1 flex gap-1 z-10">
                        <button 
                            ...
                            className="bg-blue-500 text-white p-1 rounded-sm opacity-100 hover:bg-blue-700 shadow-md"
                            title="수정"
                        >
                            <Edit3 size={14} />
                        </button>
                        <button 
                            ...
                            className="bg-red-500 text-white p-1 rounded-sm opacity-100 hover:bg-red-700 shadow-md"
                            title="삭제"
                        >
                            <Trash2 size={14} />
                        </button>
          ...
          {isAdmin && (
            <div onClick={openUploadModal} className="aspect-square bg-[#F0F0F0] border-2 border-black border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-white transition-colors group">
    ...
    <div className="min-h-screen bg-[#FDF5E6] font-sans selection:bg-[#FF00FF] selection:text-white pb-safe">
      ...
      <header className="bg-white border-b-2 border-black p-3 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
            <button 
                onClick={toggleMusic}
                className="hidden sm:flex items-center space-x-2 text-xs font-bold bg-black text-white px-2 py-1 rounded-none border border-black shadow-[2px_2px_0px_0px_rgba(200,200,200,1)] hover:bg-gray-800 transition-colors"
            >
                <Music size={12} className={isPlaying ? "animate-spin-slow" : ""} />
                <span>Now Playing: {bgmSettings.title}</span>
            </button>
            {isAdmin && (
                <button onClick={() => setShowBgmModal(true)} className="bg-white border-2 border-black p-1 hover:bg-gray-100 transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none" title="BGM 설정">
                    <Settings size={16} />
                </button>
            )}
      ...
      {showBgmModal && (
          <BGMEditModal 
            ...
            initialData={bgmSettings}
            onSave={handleBgmUpdate}
          />
      )}
    </div>
  );
